...
export class ElementAction{
    ...
    async getAttribute(elementText: string): Promise<string>{
        try{
            let attribute = await (await this.getElement()).getAttribute('value');
            //log.debug("Fetched  value as "+ text + " from " + this.byEL);
    ...
    async getAllAttribute(elementText: string): Promise<string>{
        try{
            let attribute = await (await this.getElements()).getAttribute('value');
            //log.debug("Fetched  value as "+ text + " from " + this.byEL);
           // log.debug("Fetched  value as "+ attribute + " from " + elementText);
            return attribute;
        }catch(ex){
